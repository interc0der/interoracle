    ...
    async _onMessage(event: any): Promise<void> {
        ...
        if (response != undefined && response.price != undefined) {
            ...
            if (global_index != undefined){
                if (response.symbol_id.includes('XRP')) this.XRP[0][global_index]=[response.symbol_id.split('_')[0], response.price]
                if (response.symbol_id.includes('BTC')) this.BTC[0][global_index]=[response.symbol_id.split('_')[0], response.price]
                if (response.symbol_id.includes('ETH')) this.ETH[0][global_index]=[response.symbol_id.split('_')[0], response.price]
                if (response.symbol_id.includes('XLM')) this.XLM[0][global_index]=[response.symbol_id.split('_')[0], response.price]
            }
        }
    }
...
let response = process.initializeWS(subscribe_channels, array)
