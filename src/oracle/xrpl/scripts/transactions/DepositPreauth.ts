  ...
  function DepositPreauth(tx:TransactionStream) {
    ...
    if([
        ...
        try {
            ...
            authorization.account  = tx.transaction.Account
            authorization.type = tx.transaction.Authorize ? 'grant access' : 'remove access'
            if (tx.transaction.Authorize) authorization.authorized  = tx.transaction.Authorize
            if (tx.transaction.Unauthorize) authorization.unauthorized  = tx.transaction.Unauthorize
            ...
            authorization.ledger_index    = tx.ledger_index,
            authorization.fee   = new BigNumber(dropsToXrp(tx.transaction.Fee)).toString();
