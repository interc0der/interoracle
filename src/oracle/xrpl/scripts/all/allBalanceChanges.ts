...
function allBalanceChanges(tx:TransactionStream) {
  ...
  function findType(data) {
    ...
    // offer creates are all exchanges
      ...
      } else if (data.account === tx.transaction.Destination &&
      ...
      } else if (data.account === tx.transaction.Account &&
  ...
  function parseRippleState(node:GeneralNode) {
    ...
    if (  node.NewFields 
          ...
          && node.NewFields.Balance.value === '0') return
    ...
    if ( node.NewFields ) {
        ...
        if (typeof node.NewFields.HighLimit == "object") highParty = node.NewFields.HighLimit.issuer
        if (typeof node.NewFields.LowLimit == "object") lowParty = node.NewFields.LowLimit.issuer
  ...
  if (tx.meta) {
    ...
    if (  tx.meta.TransactionResult.indexOf('tec') !== 0 
          && tx.meta.TransactionResult !== 'tesSUCCESS') return list
  ...
  tx.meta.AffectedNodes.forEach(function(affNode:any, i) {
    ...
    if (node.LedgerEntryType === 'AccountRoot') { parseAccountRoot(node) } 
    else if (node.LedgerEntryType === 'RippleState') { parseRippleState(node) }
  ...
  list.forEach(function(d) {
    ...
    if (paychan[d.account] && d.type === null) {
      ...
      if (d.account === e.Account) {
        ...
        d.paychannel_fund_final_balance = dropsToXrp(new BigNumber(e.Amount || 0)).toString()
        d.paychannel_final_balance = dropsToXrp(new BigNumber(e.Balance || 0)).toString()
