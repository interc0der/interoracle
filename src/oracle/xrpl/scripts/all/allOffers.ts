...
var allOffers = function (tx:TransactionStream) {
  ...
  if ([ 'Payment',
        'OfferCancel',
        'OfferCreate'].indexOf(tx.transaction.TransactionType) === -1) return undefined;

  if (tx.transaction.TransactionType === 'Payment' || 'OfferCancel' || 'OfferCreate') { 
    ...
    if (tx.meta) {
      ...
      if (  tx.transaction.TransactionType === 'OfferCreate'
            && tx.transaction.Expiration) offer.expiration = tx.transaction.Expiration;
      ...
      tx.meta.AffectedNodes.forEach((n, i) => {
          ...
          if (affNode.CreatedNode) {
            ...
            if (  tx.transaction.TransactionType == 'OfferCreate' 
                ...
                if (tx.transaction.OfferSequence && 
            ...
            if (typeof fields.TakerPays === 'object') { offer.taker_pays = fields.TakerPays } 
            else {
              offer.taker_pays = {
                currency: 'XRP',
            ...
            if (node.PreviousFields) {
              if (!node.PreviousFields.TakerPays) {
                ...
                offer.pays_change = new BigNumber(dropsToXrp(node.PreviousFields.TakerPays))
              ...
              if (!node.PreviousFields.TakerGets) {
                ...
                offer.gets_change = new BigNumber(dropsToXrp(node.PreviousFields.TakerGets))
