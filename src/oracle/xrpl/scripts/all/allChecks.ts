...
function allChecks(tx:TransactionStream) {
  ...
  if (tx.meta) {
    let parsedChecks = tx.meta.AffectedNodes
        .map((n, i) => {
            let node:any;
            affNode = n;
      
            if (  affNode.DeletedNode 
                  && affNode.DeletedNode.LedgerEntryType === 'Check') node = affNode.DeletedNode

            return assembleCheckData(tx, node)})
        .filter(parsed => parsed != undefined )
  

      if (Array.isArray(parsedChecks) && parsedChecks.length > 0 ) return parsedChecks
  }
}
...
function assembleCheckData(tx:TransactionStream, node:GeneralNode ) {
  ...
  if (tx.transaction.TransactionType === 'CheckCreate' || 'CheckCancel' || 'checkCash') {
    ...
    if (tx.transaction.Fee) check.fee = new BigNumber(dropsToXrp(tx.transaction.Fee)).toString();

    if (tx.transaction.TransactionType === 'CheckCancel') {
        check.create_tx_seq = tx.transaction.Sequence || tx.transaction.OfferSequence;
    ...
    if (tx.transaction.TransactionType === 'CheckCreate') {
        ...
        if (node.FinalFields) {
          if (node.FinalFields.Amount) check.amount = (dropsToXrp(node.FinalFields.Amount)).toString();
