...
function allEscrows(tx:TransactionStream) {
  ...
  if (tx.meta) {
    let parsedEscrows = tx.meta.AffectedNodes
        .map((n, i) => {
            let node:any;
            affNode = n;
      
            if (  affNode.DeletedNode 
                  && affNode.DeletedNode.LedgerEntryType === 'Escrow') node = affNode.DeletedNode

            return assembleEscrowData(tx, node)})
        .filter(parsed => parsed != undefined )
  

      if (Array.isArray(parsedEscrows) && parsedEscrows.length > 0 ) return parsedEscrows
  }
}
...
function assembleEscrowData(tx:TransactionStream, node:GeneralNode ) {
  ...
  if (tx.transaction.TransactionType === 'EscrowCreate' || 'EscrowCancel' || 'EscrowFinish') {
    ...
    if (tx.transaction.Fee) escrow.fee = new BigNumber(dropsToXrp(tx.transaction.Fee)).toString();

    if (tx.transaction.TransactionType === 'EscrowCancel') {
        escrow.create_tx_seq = tx.transaction.Sequence || tx.transaction.OfferSequence;
    ...
    if (tx.transaction.TransactionType === 'EscrowCreate') {
        ...
        if (node.FinalFields) {
          if (node.FinalFields.Amount) escrow.amount = (dropsToXrp(node.FinalFields.Amount)).toString();
