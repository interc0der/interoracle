...
function allPaymentChannels(tx:TransactionStream) {
  ...
  if([
      ...
      if (tx.transaction.Fee) paychan.fee = new BigNumber(dropsToXrp(tx.transaction.Fee)).toString()
    ...
    if ( tx.transaction.TransactionType == 'PaymentChannelCreate' ) {
      ...
      paychan.amount = node.fields.Amount ? 
          dropsToXrp(new BigNumber(node.fields.Amount)).toString() : undefined

      if ( typeof node.fields.Balance == "string") {   
          paychan.balance = node.fields.Balance ? 
              dropsToXrp(new BigNumber(node.fields.Balance)).toString() : undefined
      }
